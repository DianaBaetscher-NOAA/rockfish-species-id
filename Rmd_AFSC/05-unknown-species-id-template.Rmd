...
ind_to_toss <- genos_long_explicit_NAs %>%
  ...
  filter(n > 36) # change the missing data threshold to include more sebastalobus, potentially
...
```
159 samples removed due to >20% missing data.
...
rubias_mix <- two_col %>%
  ...
  mutate(collection = ifelse(gtseq_run == "gtseq2", "adult", collection)) %>%
...
top_assign
```
409 total samples in the mixture analysis.
That should be the total number - the indivs removed for missing data
568-159 = 409 
...
ggsave("pdf_outputs/known_sebastes_POP_dusky_sppID.png", height = 4, width = 4)
```
...
```{r}
goa <- top_assign %>%
  filter(!str_detect(indiv, "gtseq2")) # just confirmed, there were only the POPmix plates run with the rockfish spp id panel on gtseq2
# for any of the larval results, we need to remove gtseq2 samples
...
ggplot() +
  ...
  theme_bw() +
  labs(title = "GOA unknown larvae and known adult samples",
       x = "Z-score")

ggsave("pdf_outputs/GOA_unknowns_w_unknownSpp2.png", width = 6, height = 3.5)
```
...
If we make the assumption that the species at z-scores -10 to -20 are the duskies, and -5 are the POP (which already assign to POP)
```{r fully-filtered-larvae-data}
goa %>% 
  mutate(repunit = ifelse(z_score > 3 | z_score < -3, "unknown", repunit)) %>%
  group_by(repunit) %>%
  tally() %>%
  ggplot(aes(x = reorder(repunit, -n), y = n, fill = repunit)) +
  ...
  labs(title = "GOA larvae - species ID  with unknowns",
       x = "Species",
       y = "Number of larvae") +
  scale_fill_manual(values = c("darkred", "gold", "salmon", "darkorange", "dodgerblue", "lightblue", "midnightblue"))
  

ggsave("pdf_outputs/GOA_larvae_zscore_unknowns.png", width = 5, height = 3.5)
```

```{r}
goa %>% 
  #filter(z_score > -30) %>%
  mutate(repunit = ifelse(z_score > -20 & z_score < -10, "dusky?", repunit)) %>%
  mutate(repunit = ifelse(z_score < -20, "grp2", repunit)) %>%
  group_by(repunit) %>%
  tally() %>%
  ggplot(aes(x = reorder(repunit, -n), y = n, fill = repunit)) +
  geom_bar(stat = "identity") +
  ...
  labs(title = "GOA unknown larvae - species ID incorportating z-scores",
       x = "Species",
       y = "Number of larvae") +
  scale_fill_manual(values = c("darkred", "gold", "salmon", "darkorange", "dodgerblue", "lightblue", "midnightblue"))
  

ggsave("pdf_outputs/GOA_larvae_interfered_zscoreIDs.png", width = 7, height = 5)
```


And plot the z-scores associated with those species?
```{r}
goa %>%
  #filter(z_score > -30) %>%
  mutate(repunit = ifelse(z_score > -20 & z_score < -10, "grp1", repunit)) %>%
  mutate(repunit = ifelse(z_score < -20, "grp2", repunit)) %>%
  ggplot(aes(x = z_score, fill = repunit)) +
  geom_histogram() +
  facet_grid(rows = vars(repunit), scales = "free_y") +
  theme_bw() +
  theme(
    legend.position = "none",
    axis.title.x = element_text(margin = margin(t = 10)),
  ...
  labs(title = "GOA larvae",
       x = "Z-score",
       y = "Number of larvae") +
  scale_fill_manual(values = c("darkred", "gold", "salmon", "darkorange", "dodgerblue", "lightblue", "midnightblue"))
  

ggsave("pdf_outputs/GOA_larvae_interfered_zscore_dist.png", width = 3, height = 5.5)

```

```{r}
# just the z-score unknowns?
goa %>%
  filter(z_score > 3 | z_score < -3, 
         repunit != "maliger") %>%
  mutate(repunit = ifelse(z_score > -20 & z_score < -10, "grp1", repunit)) %>%
...
ggsave("pdf_outputs/GOA_larvae_unknowns_zscores.png", width = 5, height = 4)
  
```
219 known (within normal z-scores)
141 unknowns (based on z-scores)


```{r output-assignments}
goa %>%
  ungroup() %>%
  dplyr::select(-missing_loci) %>%
  write_csv("csv_outputs/mixture_assignments_20230418.csv")
...
ggsave("pdf_outputs/baseline_zscores.png", width = 10, height = 8)
```
